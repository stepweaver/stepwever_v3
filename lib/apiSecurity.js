 ...
 * @param {Object} [options.originOptions]
 ...
 * @param {boolean} [options.requireJsonContentType=true]
...
export async function withProtectedRoute(request, options = {}) {
  const {
    allowedMethods = ['POST'],
    enforceOrigin = true,
    originOptions = { allowMissingOrigin: process.env.NODE_ENV !== 'production' },
    rateLimit,
    parseJson = (r) => r.json(),
    botCheck = null,
    onBotDetected = null,
  ...
  if (enforceOrigin) {
    if (!isAllowedRequestOrigin(request, originOptions)) {
  ...
  if (
    ...
    if (!contentType.toLowerCase().includes('application/json')) {
      return {
        error: jsonErr({ error: 'Unsupported Media Type' }, 415),
      };
    }
  }
  ...
  if (schema && cleanBody == null) {
    return {
      error: jsonErr({ error: 'Invalid request' }, 400),
    };
  }
  if (schema && cleanBody != null) {
...
export function buildProtectedOptionsResponse(request, allowedMethods = ['POST']) {
  if (!isAllowedRequestOrigin(request, { allowMissingOrigin: false })) {
