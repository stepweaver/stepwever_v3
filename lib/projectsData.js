...
└── .gitignore                # Git ignore rules`,
  ...
  'rpg-dice-roller': {
    ...
    outcome: [
      'Built a portfolio piece that behaves like a real utility: visitors can use it during actual RPG sessions instead of only viewing it as a demo.',
      'Created a reusable roll engine that can support both the dedicated /dice-roller UI and future terminal commands without duplicating logic.',
      'Improved randomness quality by using the browser Web Crypto API for die results and rejection sampling to avoid uneven face distribution.',
...
Total: 14`,
  ...
  'portfolio-terminal': {
    ...
    description:
      'Browser-based terminal interface that turns a typical developer portfolio into a small operating environment for exploring content, tools, and an AI assistant through commands instead of menus.',
    imageUrl: '/images/terminal_ui.png',
    link: '/terminal',
    tags: ['Web Development', 'Terminal UI', 'Interactive', 'AI'],
    overview:
      'I built a terminal-style interface for my portfolio to turn a typical developer site into something more interactive, memorable, and useful. Instead of forcing visitors to click through a stack of static pages, the terminal lets them explore my work through commands, browse content, launch tools, chat with my AI assistant, and move through the site the way I think: through systems, not just screens.\n\nThe result is a browser-based terminal built with Next.js and React that acts as an interface layer across the portfolio. From one branded surface, visitors can view my resume, browse my Codex, launch a portfolio-native AI assistant, send a message through a guided terminal flow, check weather, roll RPG dice, and even play Zork or Blackjack. Under the hood, the terminal is driven by a modular command engine, special command modes, reusable data utilities, and shared integrations with the rest of the site.',
    problem:
      'Most developer portfolios behave like static brochures: a set of pages and links that you consume but do not really explore. I wanted something that fit how I actually think and work - a command-surface where you can move through my work, content, tools, and AI systems as if you were inside a small operating environment instead of a marketing site.',
    context:
      'This is the flagship interface layer for λstepweaver. The “users” are recruiters, clients, and curious builders who want signal quickly: what I build, how it works, and where to click next-without digging through a brochure site.',
    role:
      'I designed and built the entire terminal system: command architecture, mode flows, routing, integrations with the AI backend, Codex, contact API, weather lookup, and shared dice engine, plus the visual and interaction design that makes it feel like a cohesive environment instead of a themed menu.',
    solution:
      'I wired the terminal as a first-class interface layer on top of the portfolio. The chat command is connected to the same protected λlambda backend that powers other site chat surfaces, with channel-aware behavior so responses can adapt to the terminal context. The contact command runs a guided message flow and submits directly to the site’s contact API. A dedicated Codex mode lets visitors browse posts like a small file system: listing entries, opening posts, filtering by hashtag, and navigating back out. Commands like weather and roll bridge into shared utilities, including the same notation-aware dice engine used by the standalone RPG Dice Roller.',
    improveNext: [
      'Add lightweight intent analytics (what commands people try first; where they bounce) to sharpen the discovery path without turning the terminal into a tracking trap.',
      'Make the “some commands call live services” trust language visible directly inside `help` output for the relevant commands.',
      'Add a first-run guided walkthrough that teaches 6–8 high-signal commands without killing the “you’re in a shell” vibe.',
    ],
    features: [
      'Resume mode for browsing experience, education, and skills from the terminal',
      'Codex mode for listing, opening, and filtering posts and project write-ups like a small file system',
      'chat <message> command that connects to a shared λlambda AI backend with channel-aware behavior',
      'contact flow that collects name, email, and message inside the terminal and posts to the contact API',
      'weather command that supports city lookup, geolocation, and selection when multiple matches are found',
      'roll command wired to a shared dice engine for RPG-style notation and pool rolling',
      'Zork and Blackjack commands that embed small text-based games into the terminal surface',
      'cd-style navigation for jumping to key routes like /contact, /codex, and /dice-roller from the terminal',
      'help and clear commands for discoverability and managing the session output',
    ],
    keyFeatures: [
      {
        title: 'Command engine and modes',
        description:
          'A modular command handler routes input into discrete commands and modes, including general commands (help, clear, cd), special flows (contact), and dedicated modes (resume, Codex) that change how input is interpreted until the mode is exited.',
      },
      {
        title: 'Portfolio-native AI integration',
        description:
          'The chat command connects to the same protected λlambda backend used by other surfaces on the site. Requests are tagged with a terminal channel so the backend can adapt prompts and behavior by surface, making the assistant feel like part of the product rather than a generic API wrapper.',
      },
      {
        title: 'Content and navigation as a system',
        description:
          'Codex mode lets visitors treat posts and case studies like files: list entries, open details, filter by hashtag, and back out to the index. cd-style navigation commands bridge from the terminal into routes like /contact, /codex, and /dice-roller.',
      },
      {
        title: 'Shared utilities: dice and weather',
        description:
          'The roll command is designed to share the same notation-aware dice engine as the standalone RPG Dice Roller, and the weather flow supports city search, geolocation, and multi-result selection. These utilities make the terminal feel like a real environment rather than a pure navigation layer.',
      },
      {
        title: 'Branded, accessible UX',
        description:
          'The interface supports command history, keyboard navigation, input focus management, sanitized rich output, mobile-aware layout handling, and screen-reader help text. A custom boot sequence and Terminator-inspired presentation push the terminal into its own branded identity while staying usable.',
      },
    ],
    techStack: {
      frontend: [
        'Next.js 15 – App Router for the /terminal route and surrounding layout',
        'React 19 – Component composition and client-side state management',
        'Tailwind CSS 4 – Terminal-inspired layout, typography, and responsive behavior',
      ],
      terminal: [
        'Modular command engine with mode-based flows (resume, Codex, contact, games)',
        'Custom history handling with bounded history size for keyboard navigation',
        'DOMPurify-based HTML sanitization for safe rich terminal output',
      ],
      integrations: [
        'Shared /api/chat route for λlambda with channel-aware behavior (terminal vs other surfaces)',
        'Contact API route consumed by the guided terminal contact flow',
        'OpenWeather-backed weather lookup with city search and selection handling',
        'Shared dice engine so roll commands can reuse RPG dice logic from the dice roller',
      ],
      security: [
        'Server-side system prompts and provider selection for AI requests',
        'Same-origin and method checks on the chat route',
        'Rate limiting and timeout handling for AI calls',
        'Input sanitization and prompt-injection filtering before sending to model providers',
      ],
    },
    projectStructure: `λstepweaver v3 – Portfolio Terminal
├── app/
│   └── terminal/
│       └── page.jsx                # /terminal route and HUD-style layout
├── components/
│   └── Terminal/
│       ├── Terminal.jsx            # Core terminal component, command processing, output rendering
│       ├── TerminalShell.jsx       # Chrome and focus management wrapper
│       ├── hooks/
│       │   └── useCommandHistory.js # History state, navigation, and max-size enforcement
│       └── data/                   # Command-mode content (resume, Codex, games, etc.)
├── app/api/chat/route.js           # Protected λlambda chat endpoint shared across surfaces
├── app/api/contact/route.js        # Contact flow endpoint for terminal and site forms
└── lib/
    ├── chat/                       # λlambda chat utilities and system prompts
    └── roller.js                   # Shared RPG dice engine used by dice roller and terminal roll command`,
    outcome: [
      'Portfolio behaves like a small operating environment instead of a static brochure, with one surface tying together content, navigation, AI, utilities, and contact.',
      'Visitors can move between resume, Codex, AI chat, contact, dice, weather, and games without leaving the terminal context.',
      'Command architecture and modes make it straightforward to add new capabilities (e.g. new tools or content modes) without redesigning the interface.',
      'The same λlambda backend and dice engine are reused across multiple surfaces, demonstrating integration discipline instead of one-off demos.',
    ],
    challenges: [
      'Balancing the retro, cinematic terminal aesthetic with readability, accessibility, and mobile usability.',
      'Designing command and mode behavior that feels intuitive for both casual visitors and power users used to real terminals.',
      'Keeping the AI layer secure and grounded while making λlambda feel like part of the product instead of a raw API demo.',
      'Ensuring that terminal commands stay in sync with public documentation and sidebar copy as new capabilities are added.',
    ],
    plannedFeatures: [
      'Enforce the configured MAX_HISTORY_SIZE consistently wherever command history is appended, to keep long sessions responsive.',
      'Align all helper and placeholder text to the real chat <message> command name to avoid drift from earlier ai-command experiments.',
      'Expand the public command guide on the terminal page to surface weather, roll, and navigation commands alongside resume, Codex, games, and contact.',
      'Further refine the terminal copy to emphasize “browser-first interface with some live services” instead of implying everything is purely local.',
    ],
    conclusion:
      'This project matters to me because it shows how I like to build: practical systems wrapped in strong presentation. It is creative, but it is also structured. It combines interface design, command architecture, routing, API integration, AI orchestration, and reusable utilities in one cohesive product. It also reflects the kind of work I enjoy most: taking something familiar, finding the more interesting interaction model hiding inside it, and building that version instead.',
  },
  'neon-profile-card': {
    title: 'Profile Card',
    description:
...
lib/
  ...
  'iam-resist': {
    ...
    context:
      'Built for an editorial-style activism project that needed publishing, curation, and a small merch operation to run as one maintainable system-without a separate backend team or ops staff.',
    problem:
    ...
    improveNext: [
      'Harden internal link integrity (no broken narrative links) and add automated checks so the site can evolve without quiet 404 drift.',
      'Add more explicit “publication vs shop” prioritization controls so editorial curation always stays in the driver’s seat.',
      'Improve observability around feed freshness and webhook processing without overbuilding an ops stack.',
    ],
    outcome: [
...
└── public/                     # Static assets`,
    ...
    screenshots: [
      'Homepage hero showing the I AM [RESIST] bracketed identity system and latest editorial content.',
      'Sticker catalog grid and product detail layout, demonstrating how merch and publishing share the same visual language.',
    ],
    challenges: [
      'Balancing a serious activism tone with a merch catalog so the site still feels like a publication first, shop second.',
      'Designing a Notion content model that could power multiple feeds (voices, music, books, Intel) without turning authoring into a maintenance burden.',
      'Keeping the Stripe/Printify integration flexible enough for future products while staying simple enough for a small collective to operate.',
    ],
  },
...
└── utils/
  ...
  'silent-auction': {
    ...
    description:
      'Full-stack silent auction app built with Next.js, Supabase, and Resend, with alias-based bidding, secure enrollment, real-time updates, and hardened server-side auction flows.',
    imageUrl: '/images/silent-auction.webp',
    link: null, // replace with live URL if you have one
    githubRepo: 'stepweaver/silent-auction',
    tags: ['Next.js', 'Supabase', 'Realtime', 'Security', 'Resend'],

    overview:
      'I built this silent auction platform as a real event operations system, not just a polished fundraiser demo. The interesting part was not the catalog UI alone. It was designing a workflow that could handle bidder enrollment, alias-based privacy, real-time bidding, admin/vendor access, transactional email, and demo-safe resets without trusting the client more than necessary. Under the hood, the project uses Next.js App Router, Supabase, server-side API routes, signed cookies, CSRF protection, shared validation, and a staged hardening pass to make the app safer and more reliable under real event conditions.',

    problem:
      'A fundraiser app like this looks simple until you start working through the trust model. I needed public bidding without exposing bidder identities, separate access patterns for admins, vendors, and bidders, a live experience that still behaved well on phones and weaker connections, and email workflows that would not turn brittle during closeout. I also wanted a portfolio-safe demo mode, which meant the data access layer and operational tooling had to support both live and demo environments cleanly.',

    role:
      'I designed and built the full application: frontend, API routes, data access, validation, session handling, auction rules, email workflows, demo-mode behavior, and deployment/configuration patterns. I also revisited the project to harden weak spots in the original implementation, especially around identity, route protection, and mutation safety.',

    solution:
      'I split the app into a trust-aware architecture. Client components drive the interactive bidder experience, but sensitive actions run through server routes backed by the Supabase service role key. Browser access uses the anon key, while bidder enrollment and vendor auth are enforced with HttpOnly cookies instead of trusting localStorage or client-supplied IDs. Admin routes are protected at the proxy layer, and write operations require signed CSRF tokens.\n\nFor bidding, I extracted the auction rules into pure functions so timing, minimum increments, and closed-state checks were not buried inside UI code. The bid API validates input with Zod, reloads item and settings state on the server, uses canonical bidder identity from the database, inserts the bid, and then handles confirmation and outbid notifications as side effects.\n\nThe onboarding flow also does more work than it looks like. Alias creation includes rate limiting, email normalization, DNS-based deliverability checks, typo suggestions, uniqueness checks, and edge-case handling for cases where localStorage and cookies fall out of sync. On the client side, I used retry/timeout helpers, Realtime subscriptions, hashed state comparisons, and a few mobile-conscious decisions to keep the UX stable during event use.',

    features: [
      'Alias-based bidding with privacy-conscious enrollment and verified email handling',
      'Server-validated bid pipeline with minimum increment rules and closed-state checks',
      'HttpOnly cookie-based bidder and vendor session flows',
      'Proxy-protected admin routes and CSRF-protected mutations',
      'Supabase Realtime updates for bidding activity and dashboards',
      'Transactional email workflows for verification, confirmations, outbid alerts, and closeout',
      'Demo-mode schema support plus reset tooling for safe portfolio/public demos',
    ],
    ...
    techStack: {
      ...
      reliability_and_security: [
        ...
        'CSRF protection',
        'HttpOnly cookies',
        'Proxy-protected admin routes',
      ],
    },

    outcome: [
      'Built a real event operations app, not just a styled fundraiser demo.',
      'Hardened sensitive flows by moving identity and enrollment logic off the client and into trusted server-side routes.',
      'Separated auction rules from UI code so bid timing, increment logic, and closed-state checks are easier to reason about and test.',
      'Added operational tooling for demo resets, notifications, and closeout workflows so the app is usable in both live and portfolio-safe environments.',
      'Created a stronger full-stack reference project that shows practical security, validation, and reliability decisions under real-world event constraints.',
    ],

    challenges: [
      'The codebase reflects real tradeoffs: targeted tests exist, but coverage is not broad across the whole application.',
      'Several pages are still client-heavy, even though the most sensitive logic has been pushed server-side.',
      'The in-memory rate limiter is appropriate for this scale, but it is not the final answer for distributed production traffic.',
      'The value of the project is not that it is perfect; it is that I identified the risky parts of the workflow and deliberately hardened them without wrecking the user experience.',
    ],
  },
