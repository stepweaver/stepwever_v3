...
Total: 14`,
  ...
  'lsigil-setup': {
    title: 'λsigil Setup',
    description:
      'Built λsigil as an execution-oriented OpenClaw operator with a versioned repo, scoped identity files, and an early lead-generation workflow that could be analyzed and extended across tools.',
    imageUrl: '/images/sigil.png',
    link: '/projects/lsigil-setup',
    githubRepo: 'stepweaver/sigil',
    tags: [
      'OpenClaw',
      'AI Agents',
      'OpenRouter',
    ...
    overview:
      'λsigil started as an experiment in agent design, but the real goal was practical: I wanted an operator that could help with research, execution, and lead generation instead of acting like another disposable chat session. This project turned a generic OpenClaw instance into a named, scoped, versioned operator inside the broader λstepweaver modular agent stack, then proved that the repo itself was useful enough for another model to read and analyze directly.',
    problem:
      'The challenge was not just launching an agent. It was making one usable, inspectable, and worth operating. A generic shell is easy to start and hard to trust. I needed λsigil to have a real role, durable context, explicit operating files, a working repo, and a path toward actual execution. I was also testing a bigger question: could I build an AI operator that helps do real work, especially lead-generation work, without collapsing into vague prompt theater or runaway model cost?',
    role:
      'I orchestrated the whole build. I used ChatGPT for research, repo evaluation, prompt construction, and later direct repo analysis. I used OpenClaw as the runtime where λsigil actually operated. I created the repo, structured the operating files, verified the active workspace, defined the lead-generation profile, and kept tightening the system as I learned how OpenClaw really works. When direct API iteration got expensive, I used Cursor and its included Opus allotment to keep refining the repo and workflow. I also gave ChatGPT access to the `sigil` repo and had it follow the markdown-defined system to perform analysis, which confirmed that the repo structure was legible and useful beyond a single runtime session.',
    solution:
      'I built λsigil in line with OpenClaw\u2019s actual architecture instead of fighting it. That meant treating the file-based workspace as the source of agent identity, behavior, memory, and user context, then separating the live runtime workspace from a versioned repo I could keep shaping over time. I also chose OpenRouter as the model layer so I could work through one API surface while experimenting with different models and deciding where stronger hosted reasoning was actually worth the cost. One of the most useful outcomes was that the repo became understandable enough for another model to read, follow, and analyze directly, which made the system feel less like a one-off prompt stack and more like a portable operator definition.',
    architecture:
      'The system has two layers. The first is the OpenClaw runtime workspace, which provides the active operating files and session context. The second is the `sigil` repo, which acts as the versioned source of truth for prompts, profiles, runbooks, schemas, scripts, and the commercial lead-generation subsystem. Inside that repo, λsigil\u2019s business workflow is organized as a manual-first operating system: discovery, verification, reporting, human approval, then optional outreach drafting. The repo is not a self-running app; it is an inspectable operator playbook.',
    engineering: [
      'Used OpenClaw\u2019s file-based agent model intentionally instead of burying behavior inside one oversized hidden prompt',
      'Defined λsigil through explicit operating files such as `IDENTITY.md`, `SOUL.md`, `USER.md`, `MEMORY.md`, and `AGENTS.md`, then kept those concerns separate instead of collapsing everything into one context blob',
      'Created a dedicated `sigil` repo so the runtime workspace and the versioned source of truth could stay distinct',
      'Built a commercial lead-generation subsystem with prompts, scoring rules, source maps, schemas, validation, and a manual-first approval loop',
      'Used OpenRouter as a unified model layer so I could access multiple models through one API surface while testing where premium reasoning was worth using',
      'Paired OpenClaw execution with external orchestration: ChatGPT for research and prompt construction, Cursor for repo iteration when direct hosted usage became too expensive',
      'Verified that the repo-defined system was portable enough for ChatGPT to read the `sigil` repo, follow the markdown definitions, and perform useful analysis against the agent structure itself',
      'Validated the system by pushing λsigil into a first real lead-finding run that returned actual businesses and real data rather than toy output',
    ],
    outcome: [
      'λsigil now exists as a real operator with a versioned repo, a defined role, and a working execution model',
      'The project moved from setup into real use: λsigil produced a first lead-generation pass with actual businesses and usable findings',
      'The repo proved valuable as more than storage. ChatGPT was able to read it, follow the markdown-defined system, and perform useful analysis against the operator structure',
      'I now have a reusable pattern for building additional operators under λstepweaver without starting from scratch each time',
      'The build clarified a new skill I am actively developing: AI orchestration across research, prompt design, model selection, repo structure, and execution flow',
      'λsigil now has a clear place inside a broader modular agent stack alongside λlambda and λcerebro',
    ],
    tradeoffs: [
      'The system is usable, but strong lead verification still depends on external tooling, better enrichment, and more reliable source checking',
      'OpenClaw can do impressive work, but hosted reasoning and coding loops get expensive quickly, which makes model routing and task scoping part of the architecture',
      'The repo is an operator playbook, not a finished autonomous app; it still depends on intentional human review and approval',
      'The first lead-generation run was promising, but some business and GBP details still needed manual verification before outreach',
      'λsigil is operational now, but the long-term specialization of the agent is still being shaped through use',
    ],
    whyItMatters:
      'This project matters because it pushed me past “AI as chat” and into operator design. I did not just configure a tool. I built a system with roles, boundaries, context, versioned files, and a path toward real execution. Just as importantly, I proved the repo was legible enough for another model to read and work from, which made the system feel portable rather than trapped inside one session. It also helped define the broader λstepweaver modular agent stack: λlambda as the conversational layer, λcerebro as the memory layer I am still building out, and λsigil as the execution-oriented operator.',
    improveNext: [
      'Improve lead verification for sources like Google Business data, legitimacy checks, and mobile UX',
      'Refine model routing so expensive hosted reasoning is used more intentionally and lighter models handle simpler work',
      'Push λsigil further into real execution, especially lead qualification, reporting, and approved outreach drafting',
      'Finish λcerebro and connect it to λsigil and λlambda as the memory layer of the broader modular agent stack',
      'Keep building new operator profiles and workflows inside the repo without turning the system into a black box',
    ],
  },
