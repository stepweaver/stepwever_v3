...
Total: 14`,
  ...
  'stepweaver-dev': {
    ...
    description:
      'A custom Next.js portfolio platform that combines conventional pages, a command-driven terminal, reusable case-study rendering, Notion-backed content, and a protected AI chat layer.',
    ...
    overview:
      'stepweaver.dev is my primary portfolio application. It is not a static marketing site. The codebase combines a standard web interface with a command-line interaction layer, a reusable project-detail system, Notion-backed content routes, and shared AI chat infrastructure. The goal was to build a portfolio that reflects how I actually think and build: structured, stateful, and closer to an application shell than a brochure site.',
    problem:
      'A standard portfolio would have been easier, but it would also have flattened the work. I needed a site that could hold technical case studies, expose tools and experiments, support both normal navigation and command-driven exploration, and keep those surfaces connected instead of feeling like separate demos. The hard part was not the visual theme. It was making the site behave like a coherent system rather than a collection of one-off features.',
    role:
      'I designed and built the application end to end: information architecture, route design, terminal interaction model, project data schema, shared chat flows, API routes, Notion content integration, middleware, and deployment-oriented configuration.',
    solution:
      'I built the site as a Next.js App Router application with multiple interaction surfaces. Conventional pages handle the default browsing path. The terminal provides a second navigation and tool surface with modular command routing, mode-specific handlers, command history, contact intake, weather lookup, dice rolling, and embedded AI chat. Project pages are rendered from structured data instead of hardcoded templates, which makes the portfolio extensible. Shared hooks and API routes support both the terminal and the floating chat widget so the AI layer behaves consistently across the site.',
    architecture:
      'The application is split into route surfaces, reusable components, terminal-specific modules, and shared libraries. The terminal shell owns presentation mode and persistence, while command routing and feature logic are pushed into separate command and data modules. Content flows through dedicated API routes, including a codex endpoint backed by Notion data normalization. Security and reliability concerns are handled in middleware and shared route wrappers: CSP nonce generation, same-origin checks, request sanitization, bot protection, and rate limiting. Performance-sensitive client features such as the terminal canvas and other interactive modules are dynamically imported to keep the initial path lighter.',
    features: [
      'Command-driven terminal interface with route navigation, utilities, and mode-aware command handling',
      'Reusable project-detail renderer driven by structured project data instead of one-off page implementations',
      'Shared AI chat backend used by both the terminal and the chat widget',
      'Notion-backed codex and documentation pipeline exposed through internal API routes',
      'Stateful interaction patterns including command history, weather selection flow, contact intake flow, and local UI mode persistence',
      'Conventional route-based browsing alongside terminal-based exploration',
    ],
    engineering: [
      'Built on Next.js App Router with clear separation between route surfaces, shared libraries, and terminal modules',
      'Used dynamic imports for client-heavy interactive pieces to reduce unnecessary server-rendered weight',
      'Implemented CSP nonce middleware and additional security headers instead of relying on defaults alone',
      'Protected API routes with same-origin checks, sanitization, anti-bot fields and timing checks, and rate limiting',
      'Centralized chat behavior behind a single API route with provider fallback and prompt isolation',
      'Kept the dice engine as a shared utility with explicit tests rather than burying that logic inside the terminal component',
    ],
    outcome: [
      'Produced a portfolio that demonstrates interaction design, frontend engineering, API design, and content architecture in one codebase',
      'Created reusable infrastructure for future case studies instead of rebuilding project pages by hand',
      'Established a site structure that can hold experiments, writing, documentation, and project work without collapsing into a generic portfolio layout',
      'Made the portfolio itself a credible project artifact rather than just a container for other work',
    ],
    tradeoffs: [
      'Some key UI surfaces are still concentrated in large components, especially the generic project page and the terminal core',
      'Automated testing is present, but the clearest repo-backed coverage is around shared utility logic rather than the full interactive surface area',
      'Rate limiting can fall back to in-memory behavior when KV storage is not configured, which is practical but less durable across distributed deployments',
      'Parts of the content system are designed for growth, but some live sections are still lightly populated or in standby states',
    ],
    whyItMatters:
      'This project matters because it shows how I build when I control the whole system. The value is not that it looks unusual. The value is that the codebase combines interface design, stateful interaction, shared backend behavior, content modeling, and practical security decisions in a single coherent application. It is the best representation of my engineering style because the portfolio itself is part of the work, not packaging around it.',
  },
