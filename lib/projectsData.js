...
Total: 14`,
  ...
  overview:
    'stepweaver.dev is my main portfolio application. It combines route-based browsing, a terminal interface, structured project pages, Notion-backed content, and shared chat infrastructure. I built it as an application, not a static profile page.',

  problem:
    'A normal portfolio would have been simpler, but it would have hidden the kind of work I wanted to show. I needed one codebase for case studies, tools, experiments, writing, and command-driven navigation that still felt like one system. The problem was architecture, not styling.',

  role:
    'I built the application end to end: information architecture, routes, project data structure, terminal behavior, shared chat flows, API routes, Notion integration, middleware, and deployment configuration.',

  solution:
    'I built the site on Next.js App Router and split it into conventional pages, a terminal layer, structured project rendering, shared hooks, and internal API routes. The terminal handles command routing, history, contact intake, weather lookup, dice rolling, and AI chat. Project pages are driven by structured data instead of one-off page builds.',

  architecture:
    'The app is split into route surfaces, reusable components, terminal modules, and shared libraries. The terminal shell owns presentation state and local persistence, while command handling and feature logic live in separate modules. Content flows through internal API routes, including a codex endpoint that normalizes Notion data. Middleware and shared route utilities handle CSP nonces, same-origin checks, sanitization, bot protection, and rate limiting. Client-heavy features are dynamically imported to avoid loading everything up front.',
  ...
  whyItMatters:
    'This is the best example of how I build when I own the full system. It combines interface work, stateful interaction, backend behavior, content modeling, and practical security decisions in one application. The portfolio is part of the work.',
},
