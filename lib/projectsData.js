...
Total: 14`,
  ...
  'lsigil-setup': {
    title: 'λsigil',
    description:
      'λsigil is a local-first lead acquisition and review runtime: discovery via Brave Search or curated seeds, fetch and scoring, verification, approval gating, operator notes, reporting, and human-gated draft outreach (no automatic send). A file-driven identity and commercial-ops layer supplies rubric and constraints; the execution path lives in a TypeScript runtime and Next.js operator console over append-only NDJSON state, with optional OpenRouter reasoning after acquisition. The console and pipeline have completed a first successful web-deployed run.',
    ...
    overview:
      'Local-first lead-ops runtime in a dedicated `sigil` repo: CLI and Next.js console (`apps/console/`) over append-only NDJSON, resolver merge for notes and draft lifecycle, Brave- or seed-based discovery, optional OpenRouter after fetch, and shell adapters (OpenClaw, OpenCode, Claw Code) around the same pipeline.',
    context:
      'The operator is me. The job is internal commercial work: run market scans, triage candidates, verify signals, gate approvals, attach notes, read reports, and produce draft outreach under explicit human control. The system is built to be inspectable and local-first, not to delegate judgment to a chat session.',
    problem:
      'Lead work needs a real execution path: discover targets, inspect evidence, verify, approve or reject, record rationale, summarize state, and draft messages without treating the model as the product. That requires durable state, clear gates, and tools that stay under operator control.',
    role:
      'I own the `sigil` repo, the TypeScript runtime (`src/`), and the Next.js console. I defined operating context in markdown (profile, skills, prompts, runbooks, commercial-ops) and kept that repo as the single source of truth, with optional mirrors into OpenClaw for shell startup. I implemented CLI workflows, append-only NDJSON, resolver merge for `lead-notes.ndjson` and `draft-events.ndjson`, schemas, and tests; the console uses server-side actions only. Acquisition uses Brave or seed import; OpenRouter runs after fetch and inspection for refinement, evaluation, and optional JSON-backed drafts with deterministic fallbacks. Shell adapters let OpenClaw, OpenCode, or Claw Code drive the same pipeline without forking it.',
    solution:
      'The runtime covers the full operator loop: acquire, fetch and score, verify, approve or hold, append notes, generate daily-style reports from pipeline state, and draft outreach from templates or optional model output. Identity and rubric stay in versioned files; behavior is enforced by code, resolver output, and UI gates rather than hidden prompt state.',
    features: [
      'Execution pipeline for discovery (Brave or seeds), fetch/inspection, scoring, verification, approval gating, notes, reporting, and draft outreach',
      'Dedicated `sigil` repo as canonical source of truth; shells mirror or invoke it, they do not own a second pipeline',
      'Append-only events with resolver merge for notes and draft lifecycle; first-class drafted state for queues and review',
      'Next.js operator console over resolved local state; API keys never reach the browser',
      'Optional OpenRouter as a reasoning layer after acquisition, not as crawler or search backend',
      'Shell adapters for OpenClaw, OpenCode, and Claw Code',
      'File-driven constraint layer: profile, skills, prompts, runbooks, and `systems/commercial-ops/` docs',
    ],
    architecture:
      'Two layers: (1) identity and operating docs\u2014profile, skills, prompts, runbooks, commercial-ops\u2014that define role, rubric, and constraints; (2) execution\u2014TypeScript runtime plus Next.js console for acquisition, fetch/inspection, scoring, verification, approval, notes, draft outreach, reporting, and append-only state. In practice the pipeline is Brave-backed scans or seed import, HTTP fetch and rubric scoring, append-only events, notes and draft events merged by the resolver, optional OpenRouter only after acquisition, and the console for review and actions. OpenClaw is one supported shell, not the system boundary. The `sigil` repo stays canonical; integrations are adapters around the same runtime.',
    operationalWorkflow: [
    ...
    outcome: [
      'Working internal runtime for discovery, verification, approval-gated review, notes, reporting, and draft outreach',
      'First successful web-deployed run of the operator console against the same pipeline used locally',
      'Inspectable, versioned, local-first state instead of ad hoc chat-only workflows',
      'Deterministic tooling and gates with optional model reasoning layered on top, not replacing operator judgment',
      'One canonical runtime addressable from multiple shells without duplicating logic',
    ],
    tradeoffs: [
      'Still intentionally human-gated: approvals and outbound sends stay with the operator',
      'Local-first trade: inspectability and control over opaque automation',
      'Multi-shell adapters add maintenance surface area',
      'Not a CRM; scope stays narrow on acquisition, review, and draft prep',
      'Lead qualification rules are still tightening (see improveNext)',
    ],
    whyItMatters:
      'λsigil is the execution-oriented lane inside λstepweaver: operator-controlled tooling, explicit operating docs, append-only auditability, and optional AI where it helps scoring or drafting\u2014without the model standing in for the workflow. It is built for repeatability under real commercial constraints.',
    evidenceBar: {
      ...
      outcome:
        'Local-first lead acquisition and review: Brave/seed discovery, verification and approval gating, notes and reporting, draft outreach, NDJSON + resolver, Next.js console, optional OpenRouter, multi-shell adapters.',
    },
    improveNext: [
      'Sharpen lead qualification: contact-path quality, concrete evidence on the record, and single-offer outreach logic',
      'Tighten note-aware workflow and redraft ergonomics',
      'Improve draft quality while keeping approval human-gated',
      'Clearer drafted-state and queue visibility',
      'Reduce shell drift by keeping one canonical λsigil logic layer',
    ],
  },
