...
Demo injection: Projects section uses /projects/[slug] to inject the card into its case study page.`,
  ...
  'ai-integrations': {
    ...
    isService: true,
    serviceIntro:
      'These capabilities are available for hire. I built the λlambda LLM agent on this site-a shared multi-surface AI with persistent memory, server-only prompts, and channel-aware behavior (see the λlambda case study in the Projects section). I’m also developing λcerebro, a Postgres-backed memory layer for thoughts-capture via Slack, retrieve via MCP. Your second brain, owned by you. These can be custom-built for your needs.',
    problem:
