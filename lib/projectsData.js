...
Total: 14`,
  ...
  'portfolio-terminal': {
    ...
    evidenceBar: {
      role: 'Sole Developer',
      projectType: 'Internal Tool / Interactive UX',
      stack: ['Next.js', 'React', 'JavaScript', 'Tailwind CSS'],
      capabilityTags: ['Internal Tools', 'Operator UX', 'AI Integration'],
      trustFlags: ['Production-deployed'],
      scope: 'Ongoing feature on portfolio platform',
      outcome: 'Command-line surface for portfolio navigation and AI chat; reuses shared dice and chat logic across terminal and site.',
    },
  },
  ...
  'stepweaver-dev': {
    ...
    evidenceBar: {
      ...
      stack: ['Next.js 15', 'React', 'Tailwind CSS', 'Notion API', 'Groq', 'Vercel'],
      capabilityTags: ['Operational Web Platform', 'Automation', 'AI Integration', 'Protected API'],
      trustFlags: ['Production-deployed', 'App Router', 'Public repo'],
      scope: 'Ongoing: primary portfolio platform',
      outcome: 'Multi-surface Next.js platform: terminal, AI chat, Notion codex, shared API, route-group architecture, and production security hardening.',
    },
  },
  ...
  'n8n-automations': {
    ...
    evidenceBar: {
      ...
      projectType: 'Automation and Integration Service',
      stack: ['n8n', 'REST APIs', 'Webhooks', 'Node.js'],
      capabilityTags: ['Automation', 'Integration', 'Workflow Engineering'],
      trustFlags: ['Production flows', 'Multi-tool'],
      scope: 'Ongoing service engagement',
      outcome: 'Event-driven workflow automation, data movement, and business rule enforcement built around n8n.',
    },
  },
  ...
  'lcerebro': {
    ...
    whyItMatters:
      'It treats AI memory as infrastructure, not a UI feature. λcerebro is the memory layer in that same modular agent stack: the part responsible for durable context, retrieval, and knowledge flow between systems.',
    improveNext: [
  ...
  'llambda-llm-agent': {
    ...
    whyItMatters:
      'It turns the portfolio into a conversational interface without hiding the implementation reality. λlambda is the conversational layer in the broader λstepweaver modular agent stack: the interface layer that makes the system usable.',
    evidenceBar: {
      role: 'Sole Developer',
      projectType: 'AI Agent / Chat System',
      stack: ['Next.js', 'Groq', 'OpenAI', 'Zod', 'Rate limiting'],
      capabilityTags: ['AI-Assisted Systems', 'Protected API', 'Multi-surface'],
      trustFlags: ['Production-deployed', 'Security-hardened', 'Shared architecture'],
      scope: 'Ongoing: integrated into portfolio',
      outcome: 'Portfolio-native LLM agent shared across web chat and terminal via one protected route, with multi-provider fallback and bot detection.',
    },
  },
  ...
  'silent-auction': {
    ...
    evidenceBar: {
      role: 'Sole Developer',
      projectType: 'Full-stack Operational App',
      stack: ['Next.js', 'Supabase', 'PostgreSQL', 'Realtime', 'Auth'],
      capabilityTags: ['Realtime UX', 'Operational Web Platform', 'Auth and Trust'],
      trustFlags: ['Production-deployed', 'Real event use', 'Role-based access'],
      scope: 'Real fundraising event: live bidding under time pressure',
      outcome: 'Full-stack auction platform with real-time bidding, donor workflows, role-based access, and event administration under live event conditions.',
    },
  },
  ...
  'bill-planner': {
    ...
    role:
      'I designed and built the application end to end: schema design, route structure, server actions, validation, authentication, paycheck-window assignment logic, month propagation flow, attention and status UX, and planning UI.',
    solution:
    ...
    architecture:
      'The app uses a small relational core: ledgers, months, bill templates, income events, and bill instances. App routes handle top-level navigation. Feature folders own forms and actions. Shared helpers in `src/lib` handle metrics, paycheck-window generation, propagation, auth helpers, and validation schemas. The month workspace groups bills by paycheck rail and surfaces an attention layer alongside inline editing.',
    features: [
      'Roll a month forward from bill templates or a prior month, then review the draft and fix exceptions before saving',
      'Group bills by paycheck window with cushion and over-capacity signals so funding pressure is visible before due dates',
      'Attention flags for overdue, due soon, unassigned bills, and rows missing amounts',
      'Bill lifecycle with scheduled/due, pending, paid, and skipped, plus quick inline edits to status, paid amount, notes, and paycheck assignment',
      'Recurring bill templates with multi-weekday rules, default due behavior, optional payment links, and safe external link handling',
      'Auto-assign bills into paycheck windows by due date with manual overrides when the default assignment is wrong',
      'Month-level metrics for expected vs received income, paid expenses, and counts by attention and payment state',
      'Login rate limiting on the credentials flow to reduce brute-force risk',
    ],
    engineering: [
      ...
      'Login attempts are rate limited to reduce brute-force exposure on the credentials auth path',
    ...
    outcome: [
      ...
      'Created a practical workflow for planning a month before bills start landing: draft review, paycheck grouping, and attention signals, not just logging transactions after the fact',
    ...
    whyItMatters:
      'It shows domain modeling, workflow design, and scheduling logic applied to a real planning problem, with product-level UX for funding pressure and bill lifecycle.',
  },
  ...
  'lsigil-setup': {
    ...
    description:
      'λsigil began as an OpenClaw setup and identity exercise, but it grew into a local-first lead ops runtime for contractor prospecting: discovery, contact-path analysis, verification, approval-gated review, evidence notes, reporting, and constrained outreach drafting. Execution is a TypeScript pipeline and Next.js operator console over append-only NDJSON, with optional OpenRouter after acquisition for multi-pass review and drafts. The operator never auto-sends. The operator console has completed a first successful web-deployed run against the same runtime used locally.',
    imageUrl: '/images/sigil.png',
    link: '/projects/lsigil-setup',
    tags: [
      'TypeScript',
      'Next.js',
      'Brave Search',
    ...
    overview:
      'Operator-facing lead ops in a dedicated `sigil` repo: CLI plus Next.js console (`apps/console/`) on append-only NDJSON, resolver merge for notes and draft lifecycle, Brave- or seed-based discovery, evidence-oriented fetch and rubric logic on static HTML, optional OpenRouter after fetch for analyst-style passes and drafting, and shell adapters (OpenClaw, OpenCode, Claw Code) around one canonical pipeline.',
    context:
      'The hard part in contractor and service-business prospecting is not “finding a URL”; it is keeping discovery, evidence, contact-path judgment, and outreach discipline in one inspectable system. Setup work produced versioned operating markdown (profile, skills, prompts, runbooks, commercial-ops); the durable artifact is the runtime, not a chat session standing in for a CRM. The operator is me: run scans or seed lists, compare and rank contact paths (including forms and booking flows), verify signals, record rationale in append-only notes, read pipeline reports, and ship drafts only after approval. Optional model passes act as analyst or skeptic over the same evidence, not as an autonomous outbound agent.',
    problem:
      'Prospecting work breaks when discovery, qualification, and outreach live in disconnected tabs and ad hoc prompts. You still need a single execution path: acquire candidates, inspect static page evidence, extract and rank contact paths, verify, approve or reject, attach notes, summarize pipeline state, and draft outreach without treating the model as the system of record or auto-sending mail. That requires durable local state, deterministic gates, and tooling that keeps judgment with the operator.',
    role:
      'I own the `sigil` repo, the TypeScript runtime (`src/`), and the Next.js console. I defined operating context in markdown and kept the repo canonical, with optional mirrors into OpenClaw for shell startup. I implemented CLI workflows, append-only NDJSON, resolver merge for `lead-notes.ndjson` and `draft-events.ndjson`, schemas, and tests; the console uses server-side actions only. Acquisition uses Brave or seed import; qualification combines deterministic rubrics on fetched HTML with optional OpenRouter passes after acquisition, staged so the model can stress-test fit and contact-path choices from the evidence, not only emit drafts, with deterministic fallbacks. Shell adapters let OpenClaw, OpenCode, or Claw Code invoke the same pipeline without forking it.',
    solution:
      'What it became is a full lead-ops loop: acquire, fetch and interpret HTML into an evidence-oriented view, rank contact paths, verify, approve or hold, append notes, generate reports from pipeline state, and prepare drafts under explicit gates (templates plus optional model output bounded by commercial-ops rules). Rubric and constraints stay in versioned files; behavior is enforced by code, resolver output, and UI, not hidden prompt state.',
    features: [
      'Discovery from curated seed URLs or Brave-backed scans; fetch and inspect static HTML (no shipped headless-browser extraction layer)',
      'Contact-path extraction and evaluation against rubric signals; verification pass before outreach prep',
      'Approval gating for draft generation; outbound send stays outside the tool by design',
      'Append-only operator notes and draft events with resolver merge into each lead record',
      'Reporting from pipeline state for triage and review queues',
      'Constrained outreach drafting: one concrete problem and one wedge offer per message; templates and optional OpenRouter JSON bounded by commercial-ops rules',
      'Dedicated `sigil` repo as canonical source of truth; shells mirror or invoke it without owning a second pipeline',
      'Next.js operator console over resolved local state; Brave and OpenRouter keys never reach the browser',
      'Shell adapters for OpenClaw, OpenCode, and Claw Code',
    ],
    architecture:
      'Two layers: (1) file-driven operating context (profile, skills, prompts, runbooks, and `systems/commercial-ops/`) that define role, rubric, and constraints; (2) execution: TypeScript runtime plus Next.js console for acquisition, HTTP fetch, evidence-oriented qualification on static HTML, contact-path interpretation, verification, approval, notes, draft prep, reporting, and append-only state. Optional OpenRouter sits after acquisition for multi-pass reasoning: analyst/skeptic passes over the evidence and constrained drafting, not search, crawl, or autonomous send. OpenClaw is one supported shell, not the system boundary. Integrations are adapters around the same canonical runtime.',
    operationalWorkflow: [
      'Acquire candidates via Brave (trade + metro) or curated seed import',
      'Fetch HTML and build an evidence-oriented picture of each lead, working toward richer bundles than a single homepage skim wherever the pipeline surfaces enough context',
      'Interpret and rank contact paths: forms, booking flows, mailto/tel/social, preferring high-signal reachability over noisy public email when the page supports it',
      'Verify; run optional model-assisted review as analyst or skeptic over the evidence before approval, in addition to draft generation when warranted',
      'Approve, hold, or reject in the console or CLI; append operator notes (append-only history)',
      'Prepare constrained drafts (one problem, one offer); operator sends manually with no auto-send; reports summarize pipeline state for triage',
    ],
    engineering: [
      'Canonical `sigil` repo; runtime under `src/` and `apps/console/`',
      'Append-only NDJSON with a resolver that merges notes history and draft lifecycle into each lead',
      'Brave Search for discover flows where seed lists are not enough',
      'Qualification and rubric logic in code against fetched HTML, not a black-box “agent loop”',
      'OpenRouter optional after fetch: multi-pass evaluation (analyst/skeptic over evidence) and constrained drafting, not crawler or search backend',
      'Shell adapters for OpenClaw (workspace mirror), OpenCode (`opencode.json`, `.opencode/`), and Claw Code (`CLAW.md`, `.claw.json`)',
      'Console actions server-side only; secrets in repo-root `.env`',
    ],
    outcome: [
      'A working internal runtime for contractor/service prospecting: discovery through verification, approval-gated drafts, notes, and reporting in one toolchain',
      'First successful web-deployed run of the Next.js operator console against the same pipeline used locally, proof the console is not a mockup',
      'Inspectable, versioned, local-first state instead of chat-only improvisation',
      'Deterministic gates and rubric with optional model passes for analyst-style review and constrained drafting, not a hands-off outbound agent',
      'One canonical pipeline invocable from multiple shells without duplicating logic',
    ],
    tradeoffs: [
      'Human-gated by design: the operator approves drafts and performs any send; the system does not auto-mail prospects',
      'Qualification is grounded in static HTML fetch and explicit rules; there is no shipped browser-rendered DOM extraction path to claim',
      'Local-first trade: control and auditability over opaque fully automated outbound',
      'Multi-shell adapters add maintenance surface area',
      'Not a CRM; scope stays narrow on acquisition, review, evidence, and draft prep',
    ],
    whyItMatters:
      'λsigil is the execution lane inside λstepweaver: a concrete operator runtime for service-business prospecting (discovery, contact-path work, verification, evidence on the record, and constrained drafts), with append-only auditability and optional AI where it earns its place. It demonstrates how I build operational software that stays honest about autonomy, gates, and what the machine is allowed to do.',
    evidenceBar: {
      role: 'Sole Developer / System Designer',
      projectType: 'Lead ops runtime',
      stack: ['Next.js', 'Node.js', 'NDJSON', 'OpenRouter', 'Shell scripting'],
      capabilityTags: ['Lead Systems', 'Internal Tools', 'AI-Assisted', 'Operator UX'],
      trustFlags: ['Local-first', 'Append-only audit trail', 'Human-gated'],
      scope: 'Ongoing personal ops tooling',
      outcome:
        'Contractor lead ops: seed or Brave discovery, evidence-oriented static HTML qualification, contact-path analysis (forms/booking/mail), verification, approval-gated drafts, notes and reporting, NDJSON resolver, Next.js console (first web-deployed run done), optional OpenRouter for analyst passes and drafting, multi-shell adapters.',
    },
    improveNext: [
      'Richer evidence bundles: pull more than a thin homepage read where the fetch model allows (structured snippets, key subpages, and explicit confidence when the record is thin)',
      'Stronger contact-path interpretation: forms and booking flows as first-class reachability; deprioritize generic public email when a better path exists',
      'Multi-pass AI review over the same evidence to challenge weak fits, surface contradictions, and tighten qualification before drafting',
      'Stricter one-problem / one-offer discipline in templates and model constraints so every draft stays legible',
      'Outcome tracking and calibration: tie replies, meetings, and disqualifiers back into rubrics and prompts as the next maturity step',
      'Reduce shell drift by keeping one canonical λsigil logic layer across adapters',
    ],
  },
...
export function getProjectBySlug(slug) {
  ...
  return PROJECTS_DATA[normalizedSlug] || null;
}

export function hasRenderableProjectShape(project) {
...
export function getRenderableProjectBySlug(slug) {
  const project = getProjectBySlug(slug);
  if (!hasRenderableProjectShape(project)) return null;
  return project;
}