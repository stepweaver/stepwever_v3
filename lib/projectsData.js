...
Total: 14`,
  ...
  'lsigil-setup': {
    title: 'λsigil Setup',
    description:
      'λsigil began as an OpenClaw identity/setup project, but the work did not stop there. The foundation phase defined the agent\u2019s identity, workspace, memory boundaries, and operating rules. From that base, the project evolved into a local-first internal lead-ops system with a TypeScript runtime, CLI workflows, append-only state, and a Next.js operator console for verification, approvals, reporting, and outreach drafting.',
    ...
    overview:
      'This case study centers the foundation and first evolution of λsigil: the OpenClaw identity/workspace phase, then the runtime and lead-ops layer that grew out of it. The runtime and console work continued in the same repo; this page tells that two-phase story rather than treating setup as the whole product. The immediate user is me as operator. The broader purpose is internal execution: lead research, triage, reporting, approvals, and draft preparation under a human-gated workflow. λsigil began as an OpenClaw identity project, but the practical goal became a local-first system for structured commercial ops inside λstepweaver.',
    problem:
      'The challenge was not just launching an agent. It was making one usable, inspectable, and honest about what it can do. A generic shell is easy to start and hard to trust. I needed a real role, durable context, explicit operating files, and a path to real work—without selling prompt-shaped "autonomy" the repo does not back up. The follow-on question was whether that foundation could turn into an internal tool: structured state, review surfaces, and workflows that stay manual-first until search-backed acquisition and richer orchestration are worth wiring in.',
    role:
      'I orchestrated research, repo structure, and iteration across ChatGPT, OpenClaw, Cursor, and direct repo work. I defined λsigil in markdown operating files, kept a versioned `sigil` repo as source of truth, and mirrored the OpenClaw workspace from it so identity stayed portable. I then built the execution layer in TypeScript (CLI workflows, append-only NDJSON and a resolver, schemas, tests) and a local Next.js operator console with server-side actions over resolved local state. Discovery in the current snapshot remains manual-first and seed-driven; provider-backed search and OpenRouter-heavy reasoning are stubbed or optional next layers, not the core story yet.',
    solution:
      'I started by defining λsigil as a file-based operational agent: identity, mission, user context, memory, and operating rules lived in explicit workspace files instead of hidden prompt state. From there, I extended the project into a local-first execution repo with a TypeScript runtime, CLI workflows, append-only lead state, and a Next.js internal console for review, verification, approvals, outreach drafts, and daily reporting.',
    features: [
      'Defined the identity and operating layer through `IDENTITY.md`, `SOUL.md`, `USER.md`, `MEMORY.md`, and `AGENTS.md`',
      'Established the versioned `sigil` repo as source of truth and kept the OpenClaw workspace mirrored from it',
      'Added a runtime layer for discovery, verification, approval, draft generation, and reporting',
      'Built an internal operator console for local review instead of relying only on chat or TUI sessions',
      'Kept the system human-gated: no outbound send, no auto-approval, no fake autonomy',
    ],
    evolutionAfterSetup:
      'Phase one was making identity and workspace flat, file-driven, and inspectable. Phase two was adding code-backed workflows and local state so the same repo could carry operational work forward outside a single session. Today the README still describes manual-first mode: seeds or manual research, human approvals, drafts without automated sending. The site should not read like full autonomous outreach; it should read like a serious internal tool with clear gates and room to grow.',
    architecture:
      'The architecture split into two layers. The identity and operating layer is profile files, skills, prompts, and runbooks that define λsigil\u2019s role, tone, constraints, and domain logic. The execution layer is a local TypeScript runtime and internal Next.js console that handle lead discovery, verification, approval, outreach drafting, reporting, and append-only state. OpenClaw remains part of the identity and workspace story, but the authoritative shape of the project lives in the `sigil` repo; the workspace is a mirror for startup context, not the whole runtime engine.',
    operationalWorkflow: [
    ...
    engineering: [
      'The identity layer is explicit and versioned, but it is no longer the whole system',
      'Runtime logic lives in TypeScript and server-side console actions, not in prompts alone',
      'Source of truth is the `sigil` repo; the OpenClaw workspace mirrors it for agent startup context',
      'State is append-only and inspectable rather than hidden in chat history',
      'CLI workflows cover discovery, verification, approval, draft generation, and reporting against resolved local state',
      'The console uses server-side actions over local resolved state instead of ad hoc client-side provider calls',
      'Discovery is manual-first and seed-driven in the current snapshot; search-provider acquisition and LLM-heavy paths are the next layer, not finished core work',
      'OpenRouter and similar hooks are planned or stubbed for orchestration, not the dominant path in manual-first mode',
    ],
    outcome: [
      'λsigil now exists as both a defined operating identity and a working local repo',
      'The project moved from prompt/workspace setup into a real internal tool with discovery, verification, approvals, draft generation, and reporting',
      'The source of truth is inspectable and versionable instead of being trapped in chat sessions',
      'The system is usable for internal lead operations today, even though search-backed acquisition and agent-over-tools orchestration are still incomplete',
      'The OpenClaw identity layer stays strong; the practical value now comes from pairing it with runtime code and local state',
    ],
    tradeoffs: [
      'The current repo is operational, but still manual-first by design',
      'Search-provider-backed discovery is not yet the dominant path in the snapshot; seeds and manual research still anchor acquisition',
      'OpenRouter-style reasoning is planned or stubbed, not the core of day-to-day operation in manual mode',
      'Outreach drafts can be generated, but sending remains manual by design',
      'This is not a CRM or fully autonomous outreach system',
      'Strength is clarity, control, and inspectability—not full automation or fake autonomy',
    ],
    whyItMatters:
      'The point was never just to name an agent. The point was to turn agent logic into an inspectable system: first through explicit identity and operating files, then through a local runtime that could actually carry work forward. That shift—from prompt-shaped persona to versioned internal tool—is what makes λsigil interesting. It also situates λsigil inside λstepweaver as the execution-oriented lane, honest about what ships today and what still belongs in the next layer.',
    improveNext: [
      'Wire real search-backed acquisition once provider ergonomics and evaluation are ready',
      'Add thin agent-over-tools orchestration instead of growing opaque prompt stacks',
      'Use λsigil skills and runbooks as the reasoning layer over real tools and APIs',
      'Improve provider and config ergonomics so manual-first mode and future automation share one clear boundary',
      'Expand seed packs, fixtures, and evaluation coverage as the pipeline grows',
      'Add stronger validation for workspace versus runtime boundaries so identity files and code stay aligned',
    ],
  },
