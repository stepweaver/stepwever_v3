...
Total: 14`,
  ...
  'lsigil-setup': {
    title: 'λsigil Setup',
    description:
      'λsigil began as an OpenClaw identity/setup project, but it evolved into a local-first lead acquisition and review system. The setup phase defined the agent\u2019s identity, workspace, memory boundaries, and operating rules. From that base, the project grew into a TypeScript runtime and internal console for Brave-backed discovery or curated seeds, optional OpenRouter reasoning after acquisition, verification, approval, reporting, and human-gated outreach drafting.',
    ...
    overview:
      'Two-phase story: OpenClaw identity and file-driven workspace foundation, then a local-first TypeScript + Next.js runtime with append-only NDJSON state, optional multi-shell adapters, and one canonical `sigil` repo.',
    context:
      'The immediate user is me as operator. The broader purpose is internal commercial operations: market scans, lead triage, verification, approvals, note-taking, reporting, and outreach drafting under a human-gated workflow. λsigil started as an OpenClaw identity project, but it became a local-first operational tool.',
    problem:
      'The goal was not just to define an agent identity. It was to turn that identity into an operational workflow. λsigil needed durable context and constraints, but it also needed a real execution path: discover businesses, inspect and score them, review them in a console, verify them, approve them, and draft outreach without pretending the model alone was the system.',
    role:
      'I orchestrated research, repo structure, and iteration across ChatGPT, OpenClaw, Cursor, and direct repo work. I defined λsigil in markdown operating files, kept a versioned `sigil` repo as the only source of truth, and mirrored profile/skills/prompts into the OpenClaw workspace for startup context. I built the execution layer in TypeScript (CLI workflows, append-only NDJSON, resolver merge for notes and draft lifecycle, schemas, tests) and a local Next.js operator console with server-side actions only. Acquisition uses the Brave Search API or curated seed import; optional OpenRouter runs after fetch and inspection for query refinement, evaluation, and optional JSON-backed drafts (with deterministic fallbacks). I added shell adapters so the same repo can be driven from OpenClaw, OpenCode, or Claw Code without duplicating the pipeline.',
    solution:
      'I started by defining λsigil as a file-based operational agent instead of leaving its behavior implicit. Identity, mission, user context, and operating rules lived in versioned files instead of hidden prompt state. From there, I extended the project into a local-first runtime with CLI workflows, append-only lead state, optional AI evaluation after acquisition, first-class notes, drafted-state tracking, and a Next.js console for market scans, verification, approval, reporting, and outreach drafts.',
    features: [
      'Defined a structured identity layer through profile files, skills, prompts, runbooks, and `systems/commercial-ops/` docs',
      'Established the dedicated `sigil` repo as the source of truth; shells mirror or point at it instead of owning a second pipeline',
      'Built a runtime for discovery (Brave or seeds), fetch/inspection, scoring, verification, approval, drafting, and reporting',
      'Added a console over resolved local state with server-side actions; API keys never exposed to the browser',
      'Kept approvals human-gated and treated AI as a reasoning layer, not the product itself',
      'Added shell adapters so the same repo can be driven from OpenClaw, OpenCode, and Claw Code',
    ],
    evolutionAfterSetup:
      'Phase one was making identity and workspace flat, file-driven, and inspectable. Phase two shipped the executable pipeline: Brave-backed market scans or seed import, HTTP fetch and rubric scoring, append-only events, first-class notes (`lead-notes.ndjson`) and draft lifecycle (`draft-events.ndjson` merged by the resolver), optional OpenRouter only after acquisition, and a Next.js console for review and actions. Shell integrations stay adapters around that single runtime, not a parallel source of truth.',
    architecture:
      'The architecture split into two layers. The identity and operating layer is profile files, skills, prompts, runbooks, and commercial-ops docs that define λsigil\u2019s role, rubric, and constraints. The execution layer is a local TypeScript runtime and Next.js console that handle acquisition, fetch/inspection, scoring, verification, approval, notes, outreach drafting, reporting, and append-only state. OpenClaw is one supported shell, not the whole system. The `sigil` repo is canonical; shell integrations are adapters around the same runtime.',
    operationalWorkflow: [
      'Acquire candidates via Brave (trade + metro) or curated seed import',
      'Score observable signals on fetched HTML; refine in the console or resolver output',
      'Verify with a second pass and optional operator notes (append-only)',
      'Approve, hold, or reject in the console or CLI before outreach drafts',
      'Generate drafts (template or optional OpenRouter JSON); no outbound send',
      'Append notes and draft events; daily reports summarize pipeline state',
    ],
    engineering: [
      'Canonical `sigil` repo as source of truth; runtime under `src/` and `apps/console/`',
      'Append-only NDJSON state with a resolver that merges notes history and draft lifecycle into each lead',
      'Brave Search as the acquisition backend for market scans and discover flows',
      'OpenRouter optional for reasoning, evaluation, and drafting after acquisition, not as crawler or search backend',
      'First-class notes and drafted workflow stage for queues and UI',
      'Shell adapters for OpenClaw (workspace mirror), OpenCode (`opencode.json`, `.opencode/`), and Claw Code (`CLAW.md`, `.claw.json`)',
      'Console actions run server-side only; Brave and OpenRouter keys stay in repo-root `.env`',
    ],
    outcome: [
      'λsigil now exists as both a defined operational identity and a working local system',
      'The project moved from setup into a usable internal tool for lead acquisition, review, verification, approval, note-aware drafting, and reporting',
      'The source of truth is inspectable, versionable, and local-first instead of being trapped in chat sessions',
      'Agent logic is paired with real tools, real state, and operator controls',
      'λsigil can run through multiple shells while preserving one canonical runtime',
    ],
    tradeoffs: [
      'λsigil is operational, but still intentionally human-gated',
      'Search acquisition, evaluation, and drafting exist, but outbound sending remains manual',
      'The local-first model favors inspectability and control over hidden automation',
      'Supporting multiple shells adds flexibility, but also adds adapter-maintenance overhead',
      'The system is not a CRM and does not try to pretend it is one',
    ],
    whyItMatters:
      'The point was never just to name an agent or wire a chat shell. The point was an operator-controlled, agent-over-tools system: explicit identity and operating docs, then a deterministic runtime with optional model reasoning on top, not the model pretending to be the whole product. That situates λsigil inside λstepweaver as the execution-oriented lane with honest gates.',
    evidenceBar: {
      role: 'Sole Developer / System Designer',
      projectType: 'Lead System and Review Console',
      ...
      outcome: 'Local-first lead acquisition and review system: seed/Brave acquisition, NDJSON storage, optional OpenRouter enrichment, Next.js console, multi-shell adapters.',
    },
    improveNext: [
      'Tighten note-aware workflow and redraft ergonomics',
      'Improve AI-authored draft quality while keeping approval human-gated',
      'Add clearer drafted-state and queue visibility',
      'Reduce drift across supported shells by keeping one canonical λsigil logic layer',
      'Continue separating deterministic runtime work from optional AI reasoning',
    ],
  },
