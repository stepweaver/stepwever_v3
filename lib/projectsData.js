...
Total: 14`,
  ...
  'lsigil-setup': {
    ...
    description:
      'Local-first lead acquisition and review runtime for contractor and service-business prospecting: seed URLs or Brave-backed discovery, HTTP fetch of static HTML, contact-path extraction and rubric scoring, verification, append-only evidence notes, reporting, and human-approved draft outreach (no automatic send). File-driven commercial-ops and prompts supply constraints; execution is a TypeScript pipeline and Next.js operator console over append-only NDJSON, with optional OpenRouter after acquisition. The console has completed a first successful web-deployed run against the same runtime used locally.',
    ...
    overview:
      'Operator-facing lead ops in a dedicated `sigil` repo: CLI plus Next.js console (`apps/console/`) on append-only NDJSON, resolver merge for notes and draft lifecycle, Brave- or seed-based discovery, deterministic scoring on fetched HTML, optional OpenRouter after fetch, and shell adapters (OpenClaw, OpenCode, Claw Code) around one canonical pipeline.',
    context:
      'λsigil is built for contractor and service-business prospecting where the hard part is not “finding a URL” but holding discovery, evidence, and outreach discipline in one place. It began as an OpenClaw-oriented identity and workspace setup; that work turned into versioned operating markdown (profile, skills, prompts, runbooks, commercial-ops), and the durable product is the runtime—not a chat session pretending to be a CRM. The operator is me. The job is internal commercial work: run scans or seed lists, compare contact paths, verify signals, record rationale in append-only notes, read pipeline reports, and produce draft messages only after explicit approval. The design goal is a disciplined operator runtime with AI assistance where it helps, not a fully autonomous outbound bot.',
    problem:
    ...
    role:
      'I own the `sigil` repo, the TypeScript runtime (`src/`), and the Next.js console. I defined operating context in markdown and kept the repo canonical, with optional mirrors into OpenClaw for shell startup. I implemented CLI workflows, append-only NDJSON, resolver merge for `lead-notes.ndjson` and `draft-events.ndjson`, schemas, and tests; the console uses server-side actions only. Acquisition uses Brave or seed import; scoring and qualification run against fetched HTML in code; OpenRouter runs after acquisition for refinement, evaluation, and optional JSON-backed drafts with deterministic fallbacks. Shell adapters let OpenClaw, OpenCode, or Claw Code invoke the same pipeline without forking it.',
    solution:
      'What it became is a full lead-ops loop: acquire, fetch and score on static HTML, verify, approve or hold, append notes, generate daily-style reports from pipeline state, and prepare drafts from templates or optional model output under explicit gates. Rubric and constraints stay in versioned files; behavior is enforced by code, resolver output, and UI—not hidden prompt state.',
    features: [
      ...
      'Constrained outreach drafting: templates and optional OpenRouter JSON output bounded by commercial-ops rules',
    ...
    architecture:
      'Two layers: (1) file-driven operating context—profile, skills, prompts, runbooks, and `systems/commercial-ops/`—that define role, rubric, and constraints; (2) execution: TypeScript runtime plus Next.js console for acquisition, HTTP fetch, deterministic scoring on static HTML, verification, approval, notes, draft prep, reporting, and append-only state. Optional OpenRouter sits after acquisition for assistive reasoning and drafting, not as search or crawl. OpenClaw is one supported shell, not the system boundary. Integrations are adapters around the same canonical runtime.',
    operationalWorkflow: [
      'Acquire candidates via Brave (trade + metro) or curated seed import',
      'Fetch pages and score observable signals on static HTML; compare contact paths (tel, mailto, social, forms, booking links where present)',
      'Verify with a second pass; append evidence notes (append-only history)',
      'Approve, hold, or reject in the console or CLI before any draft is treated as ready',
      'Generate drafts from templates or optional OpenRouter JSON under constraints; operator sends manually—no auto-send',
      'Record draft events and notes; daily-style reports summarize pipeline state for review',
    ],
    engineering: [
      ...
      'OpenRouter optional after fetch for evaluation and drafting, not as crawler or search backend',
    ...
    outcome: [
      ...
      'Deterministic gates and rubric with optional model assistance for refinement and drafting, without claiming full autonomy',
    ...
    evidenceBar: {
      ...
      outcome:
        'Contractor/service lead ops: seed or Brave discovery, static-HTML fetch and rubric scoring, contact-path extraction, verification, approval-gated drafts, notes and reporting, NDJSON resolver, Next.js console (web-deployed run done), optional OpenRouter, multi-shell adapters.',
    },
    improveNext: [
      'Rank and prefer the best contact path when public email is noisy or generic—forms, booking widgets, and structured request flows often beat a scraped mailbox',
      'Treat forms and booking flows as first-class reachability signals alongside mailto/tel/social',
      'Confidence-aware qualification from static HTML evidence: explicit uncertainty instead of overclaiming when the page is thin',
      'One concrete pain cluster and one wedge offer per draft message so outreach stays legible and honest',
