...
/**
 * Internal uncached list. Used by listPublishedDocs.
 */
async function listPublishedDocsUncached() {
  const dbId = getDbId();
  if (!dbId) return [];
...
/**
 * Internal uncached fetch. Used by getDocBySlug.
 */
async function getDocBySlugUncached(slug) {
  const dbId = getDbId();
...
export async function getDocBySlug(slug) {
  ...
  const getCached = unstable_cache(
    async (s) => getDocBySlugUncached(s),
...
/**
 * Internal uncached list. Used by listFieldNotes.
 */
async function listFieldNotesUncached() {
  const dbId = getDbId();
  if (!dbId) return [];
...
/**
 * Internal uncached fetch. Used by getFieldNoteByTitle.
 */
async function getFieldNoteByTitleUncached(title) {
  const dbId = getDbId();
...
export async function getFieldNoteByTitle(title) {
  ...
  const getCached = unstable_cache(
    async (t) => getFieldNoteByTitleUncached(t),
    ["notion-meshtastic-field-note", title],
    { revalidate: NOTION_CACHE_REVALIDATE, tags: ["notion-meshtastic-docs"] }
  );
  return getCached(title);
}
