...
const CARD_COPY = {
  ...
  'lsigil-setup': {
    description:
      'Defined λsigil as a file-backed operator under λstepweaver, then built a TypeScript runtime, local Next.js console, and human-gated lead-ops loop with discovery, verification, approval, outreach drafts, and daily reporting.',
    builtFor: 'a local-first lead-ops operator inside λstepweaver',
    solved: 'the gap between agent setup and a reviewable execution loop you can actually run',
    delivered: 'identity and workspace foundation plus CLI workflows, append-only state, server-side console actions, and inspectable artifacts',
    actions: [
      'Grounded λsigil in explicit operating files, a versioned repo, and OpenClaw-aligned workspace structure',
      'Added a TypeScript runtime and Next.js console with discovery through reporting, NDJSON event storage, schemas, and tests',
    ],
  },
};
