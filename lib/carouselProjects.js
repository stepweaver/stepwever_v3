...
const CARD_COPY = {
  ...
  'lsigil-setup': {
    description:
      'OpenClaw identity and workspace foundation for λsigil, then evolution into a manual-first local lead-ops repo: TypeScript CLI, append-only state, Next.js console, and honest limits on autonomy.',
    builtFor: 'internal operator work under λstepweaver',
    solved: 'how to ground an agent in inspectable files without pretending the repo is fully autonomous',
    delivered: 'two-phase story: mirrored workspace plus source-of-truth repo, then runtime workflows with human gates',
    actions: [
      'Defined λsigil through explicit operating files and a versioned repo mirrored into OpenClaw',
      'Shipped TypeScript workflows, NDJSON state, console actions, and stubs for search-backed and model-heavy next steps',
    ],
  },
};
