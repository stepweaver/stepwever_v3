...
const CARD_COPY = {
  ...
  'bill-planner': {
    ...
    delivered: 'a month workspace with paycheck grouping, attention flags, and quick status updates',
    actions: [
  ...
  'lsigil-setup': {
    description:
      'OpenClaw setup and identity work hardened into a local-first lead ops runtime for contractor prospecting: discovery, contact-path analysis (forms, booking, mail), verification, approval-gated review, evidence notes, reporting, and constrained drafts. NDJSON + resolver, Next.js console, optional OpenRouter for analyst-style passes over the evidence, not just drafting. First web-deployed console run complete.',
    ...
    delivered: 'TypeScript pipeline plus Next.js console: evidence-oriented qualification on static HTML, resolver-backed notes and drafts, server-side actions, staged OpenRouter after acquisition, multi-shell adapters on one canonical `sigil` repo',
    actions: [
      ...
      'Sharpening toward richer evidence, stronger path interpretation, multi-pass AI review, and outcome calibration',
    ],
  },
};
