...
const CARD_COPY = {
  ...
  'lsigil-setup': {
    description:
      'Local-first lead ops runtime for contractor and service-business prospecting: seed or Brave discovery, static-HTML fetch with rubric scoring and contact-path extraction, verification, append-only evidence notes, reporting, and approval-gated draft outreach (no auto-send). Next.js console over NDJSON + resolver; first web-deployed run complete. OpenClaw was the origin context; the shipped system is the TypeScript pipeline and operator console.',
    builtFor: 'internal commercial prospecting under λstepweaver',
    solved: 'keeping discovery, qualification evidence, and outreach prep in one disciplined runtime instead of scattered tabs and prompts',
    delivered: 'TypeScript pipeline plus Next.js console: deterministic gates on fetched HTML, resolver-backed notes and drafts, server-side actions, optional OpenRouter after acquisition, multi-shell adapters on one canonical `sigil` repo',
    actions: [
      'Operator loop from discovery through verify, approve, notes, reports, and constrained drafts—human sends mail, not the tool',
      'Honest scope: static HTML scoring and rubrics in code; assistive OpenRouter after fetch; no autonomous outbound bot story',
    ],
  },
};
