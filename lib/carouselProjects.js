...
const CARD_COPY = {
  ...
  'lsigil-setup': {
    description:
      'Local-first lead-ops runtime: Brave or seed discovery, verification, approval gating, notes, reporting, and human-gated draft outreach over append-only NDJSON, with a Next.js operator console and optional OpenRouter after fetch. First web-deployed console run is done; qualification is tightening around contact paths, evidence, and single-offer outreach logic.',
    builtFor: 'internal operator work under λstepweaver',
    solved: 'repeatable lead acquisition and review with real state, gates, and tooling instead of ad hoc chat workflows',
    delivered: 'TypeScript runtime plus Next.js console: resolver-backed notes and drafts, server-side actions, multi-shell adapters on one canonical `sigil` repo',
    actions: [
      'Pipeline from discovery through verify, approve, notes, reports, and draft prep (no auto-send)',
      'Canonical `sigil` repo with OpenClaw, OpenCode, and Claw Code adapters; optional OpenRouter as post-acquisition reasoning only',
    ],
  },
};
