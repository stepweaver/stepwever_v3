...
const CARD_COPY = {
  ...
  'lsigil-setup': {
    description:
      'OpenClaw identity foundation for λsigil, evolved into a local-first lead acquisition and review system: Brave or seed acquisition, append-only NDJSON, optional OpenRouter after fetch, Next.js console, and multi-shell adapters around one canonical repo.',
    builtFor: 'internal operator work under λstepweaver',
    solved: 'how to pair a defined agent identity with a real runtime (tools, state, and human gates) instead of chat-only autonomy',
    delivered: 'two-layer architecture: file-driven identity/ops docs plus TypeScript + Next.js execution with server-side actions',
    actions: [
      'Grounded λsigil in versioned profile, skills, and commercial-ops docs with OpenClaw as one mirror shell',
      'Shipped Brave-backed scans, seed import, resolver merge for notes and draft lifecycle, and adapters for OpenCode and Claw Code',
    ],
  },
};
