import Link from 'next/link';
import CapabilityMatrix from '@/components/capabilities/CapabilityMatrix';
import SiteScrollPageBackground from '@/components/layout/SiteScrollPageBackground';
import { CAPABILITY_BUCKETS } from '@/lib/capabilitiesData';

export const metadata = {
  title: 'Capabilities | Stephen Weaver',
  description:
    'Proof-by-capability: operational web platforms, automation and integration, AI-assisted systems, protected APIs, internal tools, realtime UX, lead ops, and reporting. Each capability linked to concrete project evidence.',
  alternates: {
    canonical: 'https://stepweaver.dev/capabilities',
  },
  openGraph: {
    title: 'Capabilities | Stephen Weaver',
    description:
      'Proof-by-capability: what I can build, what I have built, and where to verify the claim.',
    url: 'https://stepweaver.dev/capabilities',
    type: 'website',
  },
};

export default function CapabilitiesPage() {
  return (
    <SiteScrollPageBackground>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        {/* Header */}
        <section className='relative border border-neon/20 bg-panel/25 p-6 sm:p-8 mb-10'>
          <div className='pointer-events-none absolute left-0 top-0 h-6 w-6 border-l-2 border-t-2 border-neon/60' />
          <div className='pointer-events-none absolute right-0 top-0 h-6 w-6 border-r-2 border-t-2 border-neon/25' />
          <div className='pointer-events-none absolute bottom-0 left-0 h-6 w-6 border-b-2 border-l-2 border-neon/25' />
          <div className='pointer-events-none absolute bottom-0 right-0 h-6 w-6 border-b-2 border-r-2 border-neon/60' />
          <div className='pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:100%_12px] opacity-10' />

          <div className='relative z-10'>
            <p className='font-ocr text-xs uppercase tracking-[0.28em] text-text-label mb-4'>
              λstepweaver // capabilities
            </p>
            <h1 className='font-ibm text-2xl sm:text-3xl md:text-4xl text-neon leading-tight mb-4'>
              Proof by capability, not by stack list.
            </h1>
            <p className='font-ibm text-base sm:text-lg text-text-secondary leading-relaxed max-w-3xl mb-6'>
              Each capability here is grounded in something I built. Click any proof chip to read the case study.
            </p>
            <div className='flex flex-wrap gap-3'>
              <Link
                href='/services'
                className='inline-flex items-center border border-neon/30 px-4 py-2 font-ocr text-xs text-text-secondary uppercase tracking-[0.12em] hover:border-neon/60 hover:text-neon transition-colors'
              >
                Services →
              </Link>
              <Link
                href='/projects'
                className='inline-flex items-center border border-neon/30 px-4 py-2 font-ocr text-xs text-text-secondary uppercase tracking-[0.12em] hover:border-neon/60 hover:text-neon transition-colors'
              >
                All Projects →
              </Link>
            </div>
          </div>
        </section>

        {/* Matrix */}
        <CapabilityMatrix capabilities={CAPABILITY_BUCKETS} />

        {/* Footer CTA */}
        <section className='mt-10 relative border border-neon/15 bg-panel/20 p-6'>
          <p className='font-ocr text-xs uppercase tracking-[0.28em] text-text-label mb-3'>
            Next step
          </p>
          <p className='font-ibm text-base text-text-secondary mb-4 max-w-2xl leading-relaxed'>
            If one of these capabilities matches what you need built, send me the problem.
          </p>
          <div className='flex flex-wrap gap-3'>
            <Link
              href='/contact?intent=brief'
              className='inline-flex items-center border border-neon bg-neon/10 px-5 py-2.5 font-ocr text-sm text-neon uppercase tracking-[0.12em] hover:bg-neon/20 transition-colors'
            >
              Send me the problem
            </Link>
            <Link
              href='/services'
              className='inline-flex items-center border border-neon/30 px-5 py-2.5 font-ocr text-sm text-text-secondary uppercase tracking-[0.12em] hover:border-neon/60 hover:text-neon transition-colors'
            >
              View Services
            </Link>
          </div>
        </section>
      </div>
    </SiteScrollPageBackground>
  );
}
